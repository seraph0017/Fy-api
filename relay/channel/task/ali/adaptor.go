...
type AliVideoMedia struct {
	...
	URL            string `json:"url"`
	ReferenceVoice string `json:"reference_voice,omitempty"` // Fy-api overlay: wan2.7-r2v reference voice.
}

// AliMediaItem is kept for older wan2.7-r2v tests and call sites.
...
type AliVideoInput struct {
	...
	ReferenceURLs  []string        `json:"reference_urls,omitempty"`  // Fy-api overlay: wan2.6-r2v 参考素材 URL 列表
...
func (a *TaskAdaptor) convertToAliRequest(info *relaycommon.RelayInfo, req relaycommon.TaskSubmitReq) (*AliVideoRequest, error) {
	...
	if strings.Contains(upstreamModel, "r2v") {
		...
		if isWan26R2V(upstreamModel) {
			...
			if len(req.Media) > 0 {
				aliReq.Input.Media = make([]AliVideoMedia, len(req.Media))
				for i, item := range req.Media {
					aliReq.Input.Media[i] = AliVideoMedia{
						Type:           item.Type,
						URL:            item.URL,
				...
				media := []AliVideoMedia{{Type: "reference_image", URL: req.InputReference}}
				if req.Metadata != nil {
					if lastURL, ok := req.Metadata["last_frame_url"].(string); ok && lastURL != "" {
						media = append(media, AliVideoMedia{Type: "reference_image", URL: lastURL})
					}
				}
