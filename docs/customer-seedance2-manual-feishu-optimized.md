...
**completed**：已完成。读取 `metadata.url`，或调用 `/v1/videos/{task_id}/content` 下载。
...
- 类型：`string`
- 必填：是
- 说明：模型名，例如 `doubao-seedance-2-0-260128`
...
- 类型：`string`
- 必填：建议传
- 说明：视频时长，例如 `"5"`、`"10"`；建议传字符串

**size**

- 类型：`string`
- 必填：否
- 说明：输出尺寸，fy-api 会转换为 Seedance 的 `resolution` + `ratio`

**metadata**

- 类型：`object` 或 JSON 字符串
- 必填：否
- 说明：透传或扩展官方 Seedance 参数
...
720p      -> resolution=720p,  ratio 默认 16:9
1080p     -> resolution=1080p, ratio 默认 16:9
```
...
- 类型：`string`
- 可选值：`image` / `image_url` / `video` / `video_url`
...
```text
image / image_url -> content.type=image_url, role=reference_image
video / video_url -> content.type=video_url, role=reference_video
```
