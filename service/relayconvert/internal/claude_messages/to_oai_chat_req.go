	...
	for _, claudeMessage := range claudeRequest.Messages {
		...
		if claudeMessage.IsStringContent() {
			...
			for _, mediaMsg := range content {
				...
				case "image":
					// Fy-api overlay: avoid nil-deref on malformed image blocks
					// such as {"type":"image"}; callers map this conversion
					// error to a client 400 instead of a server panic.
					if mediaMsg.Source == nil {
						return nil, fmt.Errorf("invalid content block: image type requires a 'source' object")
					}
