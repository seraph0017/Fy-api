...
- **默认连接**：`cn=root@8.136.146.211:58422`（`~/.ssh/tracenex_XN.pem`），`sg=root@47.236.133.70:58422`（`~/.ssh/AI_tracenex.pem`，ACR namespace `ai_transnext`），`hk=root@47.83.137.1`，`hk-test=root@47.86.175.72`，`cn-test=root@8.156.88.148:58422`，`sg-test=root@8.222.175.17`；默认源码目录 `/root/Fy-api`；均可用 `FYAPI_*` 环境变量覆盖，密码登录可用 `FYAPI_PASSWORD` 临时传入，避免把密码写进仓库
